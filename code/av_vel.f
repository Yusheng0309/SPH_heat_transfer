      subroutine av_vel(ntotal,volume,niac,pair_i,pair_j,
     &           w, vx, density, av)
...
c     density: water density of each particle                       [in]
c     av     : Average velocityof each particle                    [out]
     ...
     &        pair_j(max_interaction)
      double precision volume(maxn),w(max_interaction),
     &                 vx(dim,maxn), density(maxn), av(dim,maxn)       
      integer i,j,k,d       
...
c     for example, for the 1 dimensional shock tube problem, the E <= 0.3
      ...
      do k=1,niac       
        ...
        do d=1,dim
         dvx(d) = vx(d,i) - vx(d,j)            
         av(d,i) = av(d,i) 
     &      	       - 2*volume(j)*dvx(d)/(density(i)+density(j))*w(k)
         av(d,j) = av(d,j) 
     &	           + 2*volume(i)*dvx(d)/(density(i)+density(j))*w(k)                      
        enddo                    
      enddo  
