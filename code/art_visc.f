      subroutine art_visc(ntotal,hsml,volume,x,vx,niac,density,Tem,
...
c     dedt   : Change of specific internal energy                  [out]
     ...
     &        pair_j(max_interaction)            
      double precision hsml(maxn), volume(maxn), x(dim,maxn),
     &       vx(dim,maxn),density(maxn), Tem(maxn), w(max_interaction),
     &       dwdx(dim,max_interaction), dvxdt(dim,maxn), dedt(maxn)
      integer i,j,k,d
      double precision dx, dvx(dim), alpha, beta, etq, piv,
     &       muv, vr, rr, h, mtem, mdensity, mhsml
...
c     Calculate PIv_ij = (-alpha muv_ij c_ij + beta muv_ij^2) / density_ij

          mTem    = 0.5e0*(Tem(i) + Tem(j))
          mdesity = 0.5e0*(density(i) + density(j))
          piv     = (beta*muv - alpha*mc)*muv/mdensity              
